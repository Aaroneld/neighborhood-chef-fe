...
const AccountDrawer = (props) => {
  ...
  useEffect(() => {
    if (me) {
      axiosWithAuth()({
        ...
        .then((res) => {
          setMyList(sortList(res.data.data.Users[0].UserEvents.owned));
        })
        .catch((err) => {
  ...
  const ReffedModalContent = React.forwardRef((props, ref) => <UserEditModalContent {...props} ref={ref} />);
    ...
    <div className={classes.root}>
      ...
      >
        <section className={styleClasses.container}>
          ...
          >
            <Button className={`${styleClasses.root} ${styleClasses.notActive}`} onClick={logout}>
              <span style={{ marginRight: '5px' }}>
                <Icon height="20" icon={logoutIcon} />
              </span>
              <Typography variant="caption">Logout</Typography>
            </Button>
            <Button className={`${styleClasses.root} ${styleClasses.notActive}`} onClick={handleDrawerClose}>
              <span style={{ marginRight: '5px' }}>
          ...
          <div>
            ...
            {myList ? (
              sortList(myList).map(
                (event, ind) => (showAll || ind < 3) && <AccountEventCard event={event} key={ind} />
              )
            ) : (
